...
def predict_future_prices(test_data,symbol,period):
    #payload = {'payload': test_data.tolist(),'symbol'}
...
def predict(input_data,symbol):
    ...
    if response.status_code == 200:
        ...
        return None
def predicted_prices_component(predicted_prices):
    st.subheader("Predicted Prices")
    # Convert range to list for serialization
    day_numbers = list(range(1, len(predicted_prices) + 1))
    fig = go.Figure(data=[go.Table(
        header=dict(values=['Day', 'Price'],
                    fill_color='paleturquoise',
                    align='left'),
        cells=dict(values=[day_numbers, predicted_prices],
                   fill_color='lavender',
                   align='left'))
    ])
    st.plotly_chart(fig)
...
def main():
    ...
    if st.button("Fetch Data"):
        try:
            ...
            predicted_prices_component(predicted_prices)
